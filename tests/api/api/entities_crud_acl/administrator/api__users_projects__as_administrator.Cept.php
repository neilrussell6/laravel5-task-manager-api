...
// Test
//
// * each user's projects as administrator
//
// Endpoints
//
...
$I->comment("when we index projects for all users (excluding public users)");
$requests = [
    [ 'GET', "/api/users/{$user_admin_id}/projects", ],
    [ 'GET', "/api/users/{$user_demo_id}/projects", ],
    [ 'GET', "/api/users/{$user_subscriber_1_id}/projects", ],
    [ 'GET', "/api/users/{$user_subscriber_2_id}/projects", ],
    [ 'GET', "/api/users/{$user_admin_id}/relationships/projects", ],
    [ 'GET', "/api/users/{$user_demo_id}/relationships/projects", ],
    [ 'GET', "/api/users/{$user_subscriber_1_id}/relationships/projects", ],
    [ 'GET', "/api/users/{$user_subscriber_2_id}/relationships/projects", ],
];
...
$I->sendMultiple($requests, function($request) use ($I) {
    ...
    $I->expect("should return 2 projects");
    $I->assertCount(2, $I->grabResponseJsonPath('$.data[*]'));
    $I->seeResponseJsonPathSame('$.data[*].type', 'projects');

});

$I->comment("when we index projects for all public users");
$requests = [
    [ 'GET', "/api/users/{$user_public_1_id}/projects", ],
    [ 'GET', "/api/users/{$user_public_2_id}/projects", ],
    [ 'GET', "/api/users/{$user_public_1_id}/relationships/projects", ],
    [ 'GET', "/api/users/{$user_public_2_id}/relationships/projects", ],
];
...
$I->sendMultiple($requests, function($request) use ($I) {
    ...
    $I->expect("should return no projects");
